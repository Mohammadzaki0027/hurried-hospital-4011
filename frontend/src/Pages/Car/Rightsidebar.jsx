...
const Rightsidebar = () => {
  ...
  function fetch() {
    if (data.length === 0) {
      axios.get("https://waytoglobe.onrender.com/getcar").then((r) => {
  ...
  let handlesortprice = (value) => {
    axios.get(`https://waytoglobe.onrender.com/filterdata/${value}`).then((r) => {
      setData(r.data.data);
    });
  };

  React.useEffect(() => {
    axios.get("https://waytoglobe.onrender.com/getfilter").then((r) => {
